...
class SetAccumulatorTest : public testing::Test, public test::VectorTestBase {
 protected:
  ...
  void testPrimitive(const VectorPtr& data) {
    ...
    // The contents of the accumulator would be all the uniqueData.
  ...
  void testComplexType(const VectorPtr& data) {
    ...
    ASSERT_EQ(accumulator1.size(), uniqueValues.size());
    ...
    accumulator2.addSerialized(
        *serialized->asFlatVector<StringView>(), 0, allocator());
    ASSERT_EQ(accumulator2.size(), uniqueValues.size());
...
TEST_F(SetAccumulatorTest, integral) {
  auto data = makeFlatVector<int32_t>({1, 2, 3, 4, 5});
  testPrimitive<int32_t>(data);
  data = makeFlatVector<int32_t>({1, 2, 2, 3, 3, 4, 5, 5});
  testPrimitive<int32_t>(data);
  data = makeFlatVector<int32_t>({1, 2, 2, 3, 4, 5, 3, 1, 4});
  testPrimitive<int32_t>(data);
}
...
TEST_F(SetAccumulatorTest, array) {
 ...
 testComplexType(dataWithDuplicates);
}
